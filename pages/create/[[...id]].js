...
const ProductsManager = () => {
    ...
    const handleUploadInput = e => {
        ...
        if (files.length === 0) return dispatch({ type: 'NOTIFY', payload: { error: 'Files is not exist.' } })

        files.forEach(file => {
            if (file.size > 1024 * 1024) return err = `${file.name} file size is larger than 1mb.`
            if (file.type !== 'image/jpeg' && file.type !== 'image/png') return err = `${file.name} image format is incorrect.`
        ...
        if (imgCount + newImages.length > 5) return dispatch({ type: 'NOTIFY', payload: { error: 'Select up to 5 images.' } })
    ...
    const handleSubmit = async e => {
        ...
        if (!title || !price || !inStock || !description || !content || category === 'all' || images.length === 0)
            return dispatch({ err: "Please add all the fields." })
            ...
            <form className="row" onSubmit={handleSubmit}>
                <div className="col-md-6">
                    ...
                    <textarea name="description" id="description" cols="30" rows="4"
                        ...
                        onChange={handleChangeInput} value={description}></textarea>

                    <textarea name="content" id="content" cols="30" rows="6"
                        placeholder="Content" className="d-block my-4 w-100 p-2"
                        onChange={handleChangeInput} value={content}></textarea>
                ...
                <button type="submot" className="btn btn-dark my-5 px-4">
                    {
